...
int main(void)
{
	/*GPIOA (LED1) Configuration STRUCT*/
	PinConfig_t Led1 ={
			.Port = PORTA,
			.PinNum = PIN1,
	...
	/*Data to be sent to USART2*/
	uint8_t G_SrcArray[2] = {0,0b00000111};
	...
	/*Enable RCC Clock line for GPIOA*/
	...
	//DMA_SetCallBack(DMA_NUM_1, DMA_CHANNEL_7, DMA_CHANNELx_TRANSFER_COMPLETE_FLAG, Notification_DMA);
	DMA_Start_(&DMA1_Init, (uint32_t)&G_SrcArray[0] , (uint32_t)&GPIOA->GPIOx_ODR , 1 );
	...
	DMA_Start_(&DMA1_Init, (uint32_t)&G_SrcArray[1] , (uint32_t)&GPIOA->GPIOx_ODR , 1 );
